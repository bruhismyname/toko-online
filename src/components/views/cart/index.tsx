  ...
  useEffect(() => {
    const checkUser = async () => {
      try {
        ...
        showNotification("Failed to check session.", "error");
      }
    };
    checkUser();
  ...
  const handleIncreaseQty = async (product_id: string, cart_id: string, stock_id: string) => {
    try {
      ...
      if (res.ok) {
        ...
        showNotification("Failed to increase quantity.", "error");
      }
    } catch (error) {
  ...
  const handleDecreaseQty = async (product_id: string, cart_id: string, stock_id: string) => {
    try {
      ...
      if (res.ok) {
        ...
        showNotification("Failed to decrease quantity.", "error");
      }
    } catch (error) {
  ...
  const handleCheckout = async (cartItemId: string, userId: string) => {
    try {
      ...
      if (res.ok) {
        ...
        showNotification("Unexpected error during checkout.", "error");
      }
    } catch (error) {
                      ...
                      <div className="flex gap-6">
                        ...
                        <div className="flex-1 flex flex-col">
                          <div className="flex-1">
                            <h2 className="text-xl font-semibold text-gray-900 mb-2">{item.product.name}</h2>
                            ...
                            <p className="text-gray-600 mb-1">
                              Size: <span className="font-semibold">{item.stock.size}</span>
                            </p>
                            ...
                            <div className="flex items-center gap-3 mt-4">
                              ...
                              <span className="w-14 text-center font-semibold text-gray-900 text-lg">{item.qty}</span>
                              <button
                          ...
                          <div className="mt-4">
                            ...
                            >
                              ...
                              >
                                <path
                                  fillRule="evenodd"
                                  d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a 1 1 0 00-1 1v6a1 1 0 102 0V8a 1 1 0 00-1-1z"
                                  clipRule="evenodd"
                                />
                              </svg>
                              Remove
